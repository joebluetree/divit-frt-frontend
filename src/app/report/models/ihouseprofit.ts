...
//Remark : all data variable used in House Profit Report Volume
//version : v1 - 06/03/2026

export interface iHouseProfit {
  mbl_id: number;
  mbl_hbl_id: number;
  mbl_cfno: number;
  mbl_mode: string;
...
export interface iHouseProfit_Search {
  ...
  mbl_consignee_name: string;
  mbl_handled_name: string;
  mbl_client_type: string;

  rec_company_id: number;
  rec_branch_id: number;
}

export interface iHouseProfitModel {
  selected_row_id: number;
  records: iHouseProfit[],
  errorMessage: string,
  searchRecord: iHouseProfit_Search,
  pageRecord: iPage,
  sort_column: string;
  sort_order: string;
};
